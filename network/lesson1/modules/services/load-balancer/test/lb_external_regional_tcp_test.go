...
func TestExternalRegionalTCPLoadBalancer(t *testing.T) {
	t.Parallel()
	...
	testCtx := createTestContext(t, "../../", "load-balancer/examples/external/regional/tcp")
	...
	test_structure.RunTestStage(t, "test_load_balancer_via_public_get_request", func() {
		...
		_, err := doWithRetryAndTimeoutE(t, "Attempting to Curl", SSHMaxRetries, SSHSleepBetweenRetries, SSHTimeout, func() (string, error) {
			...
			resp, err := http.Get(fmt.Sprintf("http://%s:5222/", loadBalancerIP))
			if err != nil {
				log.Fatalln(err)
			}
