...
func TestInternalRegionalTCPLoadBalancer(t *testing.T) {
	t.Parallel()
	...
	testCtx := createTestContext(t, "../../", "load-balancer/examples/internal/regional/tcp")
	...
	test_structure.RunTestStage(t, "test_load_balancer_from_ssh_host", func() {
		...
		_, err := doWithRetryAndTimeoutE(t, "Attempting to SSH", SSHMaxRetries, SSHSleepBetweenRetries, SSHTimeout, func() (string, error) {
			...
			output, err := ssh.CheckSshCommandE(t, bastionHost, fmt.Sprintf("curl -I %s:5222", loadBalancerIP))
			if err != nil {
				t.Fatal(err)
			}
